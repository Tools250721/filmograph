...
import { getMyWatchlist } from "../../data/myPageAPI";
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setWatchlistMovies(Array.isArray(data) ? data : []);
      } catch (error) {
  ...
  if (isLoading) {
    return (
      <div className={styles.pageWrapper}>
        <Header_2 />
        <div className={styles.loadingMessage}>위시리스트를 불러오는 중...</div>
      </div>
    );
  }
    ...
    <div className={styles.pageWrapper}>
      ...
      <div className={styles.contentWrapper}>
        <h2 className={styles.title}>
          보고싶어요한 영화 ({watchlistMovies.length})
        </h2>
